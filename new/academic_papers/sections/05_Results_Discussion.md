...
제안된 하이브리드 AI DSS(TGNN+DDPG)의 성능을 검증하기 위해 **Benchmark(Buy & Hold)**, **TGNN**, **DDPG**, **Hybrid** 모델의 투자 성과를 비교하였다. 실험은 **Test 기간(2021–2025)**의 데이터를 기반으로 수행되었으며, 5개의 독립 시드($S = \{0, 42, 123, 456, 789\}$)를 사용한 반복 실험의 **평균(Mean) ± 표준편차(Std)**로 결과를 보고한다.
