...
이와 같이 RL 기반 DSS 연구들은 의사결정의 자동화 수준을 높였지만, 대부분이 단일 정책 학습에 국한되어 있으며 복수 모델 결합 시 발생하는 시드 의존성(Seed Dependency) 증가와 재현성 저하 문제를 간과하였다. 본 연구는 이러한 한계를 극복하기 위해 TGNN 기반 관계 학습과 DDPG 기반 정책 최적화를 고정 알파(Fixed Alpha, α = 0.5) 균등 앙상블로 통합하여, 재현성이 강력히 보장된 하이브리드 기법을 제안한다.
...
| ---------------- | --------------------------- | ---------------------------------------------------- |
| AI DSS 구조      | 단일 모델 예측형 DSS        | TGNN–DDPG 고정 알파(Fixed Alpha) 균등 앙상블 DSS     |
| 정책 학습        | 수익률 중심                 | 리스크 조정 보상 + 결정론적 균등 앙상블 최적화       |
...
| 해석 가능성(XAI) | 일부 후처리 분석            | Temporal & Stock-Level Attention 기반 설명 제공      |

요약하면, 본 연구는 AI 모델링과 DSS 시스템 통합을 유기적으로 결합하여, 예측 모듈과 정책 제어 모듈이 균등 앙상블 기반으로 작동하는 지능형 AI DSS 프레임워크를 제안한다. 또한, 결정론적 알고리즘을 도입하여 학술적 재현성을 보장함으로써 신뢰할 수 있는 연구 결과를 제시한다.
