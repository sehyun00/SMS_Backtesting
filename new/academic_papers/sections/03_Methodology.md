...
graph TD
    subgraph Input
        ...
        H["Horizon (1)"] --> H_Emb["Horizon Embedding (Dim)"]
        A["Adj Matrix (B, N, N)"]
    end
    ...
    subgraph "TGNN Path (Relation-Aware)"
        ...
        A --> TGNN_Enc
        TGNN_Enc --> |"Multi-Head Prediction"| TGNN_Head["Multi-Task Heads<br/>(1M, 3M, 6M, 12M)"]
        TGNN_Head --> |"Score (N)"| TGNN_Soft[Softmax / Temp]
        TGNN_Soft --> |Weights| W_TGNN[TGNN Weights]
    end

    subgraph "DDPG Path (Policy Optimization)"
        Ft --> DDPG_Actor["DDPG Actor (Shared Encoder)"]
        DDPG_Actor --> |Weights| W_DDPG[DDPG Weights]
    end

    subgraph "Dynamic Ensemble (Horizon-Aware)"
        TGNN_Enc --> Concat[Concat Context]
        DDPG_Actor --> Concat
        H_Emb --> Concat
        Concat --> |"(N, Hidden)"| Pool["Global Pool Head<br/>(Deep Sets Aggregation)"]
        Pool --> |"Global Context"| Alpha_Net[Alpha MLP]
...
여기서 $A$는 인접행렬, $D$는 차수행렬, $H$는 노드 특징행렬, $W$는 학습 가중치이다. 본 모델은 안정적 학습을 위해 잔차 연결(Residual Connection)과 배치 정규화(Batch Normalization)를 적용하였다.
...
여기서 $\alpha$는 Global Pooling Network를 통해 산출된 동적 가중치(Dynamic Weight)이다.
