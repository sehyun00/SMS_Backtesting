...
@dataclass
class Config:
    """학습 및 백테스트 설정 (README 하이퍼파라미터 기준)"""
    ...
    FINETUNED_MODEL_PATH: Path = Path(__file__).parent / "best_hybrid_finetuned.pth"
    ...
    TAU: float = 0.001  # README standard (Target network soft update)
    ENTROPY_COEF: float = 0.01  # Entropy 정규화 계수

    # Fine-tuning 설정
    FINETUNE_EPISODES: int = 50
    FINETUNE_BATCH_SIZE: int = 32
    FINETUNE_BUFFER_SIZE: int = 128
...
class ModelManager:
    ...
    def save_model(self, agent: HybridAgent, finetuned: bool = False):
        """모델 저장"""
        save_path = (
...
class BacktestRunner:
    ...
    def run_hybrid_strategy(
        ...
        for i, window in enumerate(test_windows):
            ...
            portfolio.update(returns, window["date"])
...
class WorkflowManager:
    ...
    def run_comparison_mode(self):
        ...
        # Fine-tuned 모델 저장
        self.model_manager.save_model(test_agent, finetuned=True)
